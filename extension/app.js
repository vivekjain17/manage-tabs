...
/**
 * fetchUserFirstName() — returns the first name of the signed-in Chrome user
 * by reading the profile email via the identity API.
 * Returns null silently if not available.
 */
async function fetchUserFirstName() {
  if (!chrome.identity?.getProfileUserInfo) return null;
  // Try accountStatus ANY (works even when sync is off), then fall back to SYNC
  for (const details of [{ accountStatus: 'ANY' }, {}]) {
    try {
      const info = await new Promise((resolve, reject) => {
        chrome.identity.getProfileUserInfo(details, (i) => {
          if (chrome.runtime.lastError) reject(chrome.runtime.lastError);
          else resolve(i);
        });
      });
      if (!info?.email) continue;
      const local = info.email.split('@')[0];   // "vivek.jain"
      const first = local.split(/[._+\-]/)[0];  // "vivek"
...
async function renderStaticDashboard() {
  ...
  // Append first name if the user is signed into Chrome
  fetchUserFirstName().then(name => {
    if (name && greetingEl) greetingEl.textContent = getGreeting() + ', ' + name;
  }).catch(() => { /* identity unavailable — greeting stays plain */ });
...
document.addEventListener('click', async (e) => {
  ...
  if (action === 'close-tabout-dupes') {
    ...
    showToast('Closed extra Manage Tabs tabs');
    return;
  }
